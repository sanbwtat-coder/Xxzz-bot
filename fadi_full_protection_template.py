...
# استبدل ملف protection_template.py القديم بهذا الملف داخل مشروع الصانع
...
DEFAULT = {
    ...
    "cooldowns": {}, "robbers": {}, "locks": {}, "lock_actions": {}, "ranks": {}, "warns": {},
...
CHANTS = [("ترتيلة بيزنطية", "ارفع رابط ترتيلة صحيح هنا"), ("ترتيلة أرثوذكسية عربية", "ارفع رابط ترتيلة صحيح هنا")]
...
def source_markup_raw(): return str({"inline_keyboard": [[{"text": SOURCE_NAME, "url": f"https://t.me/{DEV_USERNAME}", "style":"danger"}]]}).replace("'", '"')
...
def is_admin_msg(message): return message.from_user and (is_group_admin(message.chat.id, message.from_user.id) or rank_level(message.chat.id, message.from_user.id) >= 1)
def is_creator_basic(message): return message.from_user and (is_group_admin(message.chat.id, message.from_user.id) or rank_level(message.chat.id, message.from_user.id) >= 5)
...
def cooldown(uid, key, seconds):
    uid = sid(uid); now = int(time.time()); data["cooldowns"].setdefault(uid, {}); last = data["cooldowns"][uid].get(key, 0)
...
def is_subscribed(user_id):
    ...
    try: return bot.get_chat_member(ch, user_id).status in ["member", "administrator", "creator"]
    except: return True
...
def save_media(message):
    ...
    if message.content_type not in ["photo", "video", "sticker", "animation"]: return
    gid = sid(message.chat.id); data["media_messages"].setdefault(gid, []); data["media_messages"][gid].append({"id": message.message_id, "type": message.content_type}); data["media_messages"][gid] = data["media_messages"][gid][-1000:]; save_json(DATA_FILE, data)
...
def punish(message, lock_key):
    ...
    if action == "mute":
        try: bot.restrict_chat_member(message.chat.id, message.from_user.id, can_send_messages=False); data["muted"].setdefault(gid, []); data["muted"][gid].append(sid(message.from_user.id)); save_json(DATA_FILE, data)
        except: pass
    elif action == "kick":
        try: bot.ban_chat_member(message.chat.id, message.from_user.id); bot.unban_chat_member(message.chat.id, message.from_user.id); data["kicked"].setdefault(gid, []).append(sid(message.from_user.id)); save_json(DATA_FILE, data)
        except: pass
    elif action == "restrict":
        try: bot.restrict_chat_member(message.chat.id, message.from_user.id, can_send_messages=False, until_date=int(time.time())+3600); data["restricted"].setdefault(gid, []).append(sid(message.from_user.id)); save_json(DATA_FILE, data)
        except: pass
...
def detect_violation(message, text):
    ...
    content_map = {"photo":"photo", "video":"video", "sticker":"sticker", "animation":"animation", "document":"document", "voice":"voice", "audio":"audio", "contact":"contact", "video_note":"video_note"}
    key = content_map.get(message.content_type)
...
def add_quiz(chat_id, question, choices, answer):
    ...
    kb = {"inline_keyboard": [[blue(c, f"quiz_{game_id}_{c}")] for c in choices[:3]]}
...
def list_top(message):
    ...
    out = "- ترند المجموعة\n\n" + "".join([f"{i}- {display_user(uid)} ↢ {count} رسالة\n" for i,(uid,count) in enumerate(items,1)])
...
def handle_waiting(message, text):
    ...
    if step == "add_reply_answer": data["replies"].setdefault(gid, {})[w["word"]] = text; data["waiting"].pop(uid,None); save_json(DATA_FILE,data); bot.reply_to(message,"تم إضافة الرد"); return True
    if step == "del_reply": data["replies"].setdefault(gid, {}).pop(text, None); data["waiting"].pop(uid,None); save_json(DATA_FILE,data); bot.reply_to(message,"تم حذف الرد"); return True
    ...
    if step == "add_tag_user": data["tags"].setdefault(gid, {})[w["name"]] = text; data["waiting"].pop(uid,None); save_json(DATA_FILE,data); bot.reply_to(message,"تم إضافة التاك"); return True
    if step == "whisper_text":
        wid = str(int(time.time())) + str(random.randint(100,999)); data["waiting_games"][wid] = {"type":"whisper", "from":message.from_user.id, "to":w["to"], "text":text}; data["waiting"].pop(uid,None); save_json(DATA_FILE,data)
        return raw_send(message.chat.id, f"• الهمسه لـ {display_user(w['to'])}\n• من ← {message.from_user.first_name}", {"inline_keyboard": [[blue("رؤية الهمسة", f"whisper_{wid}")]]}) or True
    return False
...
def callbacks(call):
    ...
    if call.data == "commands": return raw_edit(call.message.chat.id, call.message.message_id, "- اختر قسم الاوامر", commands_kb())
    ...
    if call.data.startswith("quiz_"):
        ...
        game = data.get("waiting_games", {}).get(parts[1]); answer = parts[2]
        if not game:
            msg = bot.send_message(call.message.chat.id, "انتهت اللعبة"); delete_after(call.message.chat.id, msg.message_id, 2); return
        if answer == game["answer"]:
            uid = sid(call.from_user.id); data["points"][uid] = data["points"].get(uid,0)+1; data["waiting_games"].pop(parts[1], None); save_json(DATA_FILE, data); return bot.send_message(call.message.chat.id, f"✅ إجابة صحيحة\nربحت نقطة\nنقاطك: {data['points'][uid]}")
...
def handler(message):
    register(message); save_media(message)
    if message.chat.type != "private" and not check_sub(message): return
    text = message.text or message.caption or ""; gid = sid(message.chat.id)
    if handle_waiting(message, text): return
    ...
    if text.startswith("قفل ") or text.startswith("فتح "):
        ...
        for word,val in [("بالكتم","mute"),("بالطرد","kick"),("بالتقييد","restrict")]:
            if word in rest: lock_action=val; rest=rest.replace(word,"").strip()
        key = LOCK_TYPES.get(rest)
        if not key: return bot.reply_to(message, "هذا القفل غير موجود")
        data["locks"].setdefault(gid, {})[key] = action == "قفل"; data["lock_actions"].setdefault(gid, {})[key] = lock_action; save_json(DATA_FILE, data)
    ...
    if text == "نزلني": data["ranks"].setdefault(gid, {}).pop(sid(message.from_user.id), None); save_json(DATA_FILE,data); return bot.reply_to(message, "تم تنزيلك من جميع الرتب")
    if text == "كشف" and message.reply_to_message:
        u = message.reply_to_message.from_user; uid = sid(u.id); return bot.reply_to(message, f"- الاسم: {u.first_name}\n- الايدي: <code>{u.id}</code>\n- اليوزر: @{u.username or 'ماكو'}\n- رتبة البوت: {get_rank(message.chat.id,u.id)}\n- الرسائل: {data['messages'].get(gid,{}).get(uid,0)}\n- النقاط: {data['points'].get(uid,0)}")
    if text == "صلاحياتي":
        try:
            m = bot.get_chat_member(message.chat.id, message.from_user.id); attrs=[("حذف الرسائل","can_delete_messages"),("حظر المستخدمين","can_restrict_members"),("تثبيت الرسائل","can_pin_messages"),("دعوة المستخدمين","can_invite_users"),("تغيير معلومات المجموعة","can_change_info")]
        ...
        except: return bot.reply_to(message,"ما اكدر اجيب صلاحياتك")
    if text == "اضف رد":
        if not is_admin_msg(message): return
    ...
    if text == "الردود": return bot.reply_to(message, "• الردود المضافة:\n\n" + ("\n".join("- "+x for x in data["replies"].get(gid,{}).keys()) or "ماكو ردود"))
    if text == "اضف تاك":
        if not is_admin_msg(message): return
    ...
    if text.startswith("مسح تاك "):
        if not is_admin_msg(message): return
        name = text.replace("مسح تاك ", "", 1).strip(); data["tags"].setdefault(gid, {}).pop(name, None); save_json(DATA_FILE,data); return bot.reply_to(message,"تم حذف التاك")
    if text.startswith("اضف نقاط ") and message.reply_to_message:
        if not is_admin_msg(message): return
        amount = int(re.findall(r"\d+", text)[0]) if re.findall(r"\d+", text) else 0; uid = sid(message.reply_to_message.from_user.id); data["points"][uid] = data["points"].get(uid,0)+amount; save_json(DATA_FILE,data); return bot.reply_to(message, f"تم إضافة {amount} نقطة")
    ...
    if text == "ضع رابط":
        ...
        except: return bot.reply_to(message,"ما اكدر اسوي رابط")
    ...
    if text.startswith("مسح ") and re.search(r"\d+", text):
        ...
        return
    if text == "قائمه المنع": return bot.reply_to(message, "\n".join(data["blocked_words"].get(gid, [])) or "ماكو كلمات ممنوعة")
    ...
    if text in ["زرف", "سرقه", "سرقة"] and message.reply_to_message:
        ...
        if not ok: return bot.reply_to(message, f"انتظر {left} ثانية")
        v=message.reply_to_message.from_user; amount=min(bank(v.id), random.randint(50,500)); save_bank(v.id, bank(v.id)-amount); save_bank(message.from_user.id, bank(message.from_user.id)+amount); data["robbers"][sid(message.from_user.id)] = data["robbers"].get(sid(message.from_user.id),0)+1; save_json(DATA_FILE,data); return bot.reply_to(message, f"زرفته {amount}$")
    if text.startswith("استثمار ") or text.startswith("حظ ") or text.startswith("مضاربه "):
        ...
        if text.startswith("استثمار "): profit=int(amount*random.randint(1,15)/100); save_bank(message.from_user.id, bal+profit); return bot.reply_to(message, f"ربحت {profit}$")
    ...
    if text == "توب الفلوس": return bot.reply_to(message, "\n".join([f"{i+1}- {display_user(uid)}: {m}$" for i,(uid,m) in enumerate(sorted(data["money"].items(), key=lambda x:x[1], reverse=True)[:10])]) or "ماكو")
    if text == "توب الحراميه": return bot.reply_to(message, "\n".join([f"{i+1}- {display_user(uid)}: {c}" for i,(uid,c) in enumerate(sorted(data["robbers"].items(), key=lambda x:x[1], reverse=True)[:10])]) or "ماكو")
    if text == "المالك":
        try:
            owner=next((a.user for a in bot.get_chat_administrators(message.chat.id) if a.status=="creator"), None)
    ...
    if text.startswith("اضف لقب ") and message.reply_to_message:
        if not is_admin_msg(message): return
        title=text.replace("اضف لقب ","",1).strip(); data["titles"].setdefault(gid,{})[sid(message.reply_to_message.from_user.id)] = title; save_json(DATA_FILE,data); return bot.reply_to(message,"تم إضافة اللقب")
    ...
    if text == "ز":
        ids=[x for x in data["messages"].get(gid,{}).keys() if x != sid(message.from_user.id)]
        if not ids: return bot.reply_to(message,"ماكو أعضاء")
        p=random.choice(ids); data["marriages"][sid(message.from_user.id)] = p; save_json(DATA_FILE,data); return bot.reply_to(message, f"مبروك زوجتك {display_user(p)}")
    ...
    if text == "همسه" and message.reply_to_message:
        data["waiting"][sid(message.from_user.id)] = {"step":"whisper_text", "chat":gid, "to":message.reply_to_message.from_user.id}; save_json(DATA_FILE,data); return bot.reply_to(message, f"• تم تحديد الهمسة لـ {message.reply_to_message.from_user.first_name}\n• اكتب الهمسة")
    ...
    if text == "تراتيل": name,url=random.choice(CHANTS); return bot.reply_to(message, f"🎼 {name}\nارفع روابط التراتيل الصحيحة داخل القائمة حتى يرسلها صوت.")
    ...
    if text == "الاسرع": word=random.choice(FAST_WORDS); data["fast_games"][gid]=word; save_json(DATA_FILE,data); return bot.reply_to(message, f"⚡ لعبة الاسرع\n\nاكتب:\n{word}")
