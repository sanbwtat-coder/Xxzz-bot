...
import os, json, time, re, random, requests, threading
import telebot
import yt_dlp
...
DEFAULT = {
    "users": {}, "groups": {}, "locks": {}, "ranks": {}, "messages": {},
    "points": {}, "bank": {}, "waiting_games": {},
...
def bot_info():
    return maker_data().get("bots", {}).get(BOT_ID, {"plan": "free", "force_channel": OWNER_FORCE_CHANNEL})
...
FAST_WORDS = ["تفاحة", "سيارة", "عراق", "قلم", "ماء", "كتاب"]

def source_markup():
    return {"inline_keyboard": [[red(SOURCE_NAME, url=f"https://t.me/{DEV_USERNAME}")]]}
...
def handler(message):
    ...
    if text == "سورس":
        return raw_send(message.chat.id, SOURCE_NAME, source_markup())
    ...
    if text.startswith("يوت ") or text.startswith("تشغيل "):
        ...
        try:
            ...
            with open(file_path, "rb") as audio:
                bot.send_audio(message.chat.id, audio, title=title, performer="Song fadi", caption=f"🎧 {title}", reply_to_message_id=message.message_id, reply_markup=source_markup())
            try:
                os.remove(file_path)
            except:
                pass
    ...
    if text == "ا":
        ...
        txt = f"- الاسم: {message.from_user.first_name}\n- الايدي: <code>{message.from_user.id}</code>\n- الرتبة: {get_rank(message.chat.id, message.from_user.id)}\n- الرسائل: {msgs}\n- النقاط: {pts}"
        return bot.reply_to(message, txt)

    if text == "لغز":
        q = random.choice(PUZZLES); return add_quiz(message.chat.id, "لغز:\n" + q[0], q[1], q[2])
    if text == "عربي":
        q = random.choice(ARABIC_Q); return add_quiz(message.chat.id, "عربي:\n" + q[0], q[1], q[2])
    if text == "اعلام":
        q = random.choice(FLAGS); return add_quiz(message.chat.id, q[0], q[1], q[2])
    if text == "سيارات":
        q = random.choice(CARS); return add_quiz(message.chat.id, q[0], q[1], q[2])
    if text in ["مشاهير", "المشاهير"]:
        q = random.choice(FAMOUS); return add_quiz(message.chat.id, q[0], q[1], q[2])
