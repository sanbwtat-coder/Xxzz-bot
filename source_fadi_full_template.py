...
# SOURCE FADI - Full Protection + Music Template
# ارفع هذا الملف مكان protection_template.py داخل مشروع الصانع
...
DEFAULT = {
    ...
    "id_template": {}, "links": {}, "media_messages": {}, "pending_whispers": {},
...
RANK_LEVEL = {
    "عضو": 0, "مميز": 1, "ادمن": 2, "مشرف": 2, "مدير": 3,
    "منشئ": 4, "منشئ اساسي": 5, "منشئ أساسي": 5, "مالك": 6
}
...
CHANTS = [
    ("ترتيلة بيزنطية", "ضع رابط MP3 هنا"),
    ("ترتيلة أرثوذكسية عربية", "ضع رابط MP3 هنا"),
    ("Catholic Chant", "ضع رابط MP3 هنا")
]
PUZZLES = [
...
def bot_info():
    return maker_data().get("bots", {}).get(BOT_ID, {
        "plan": "free",
...
def owner_id():
    try:
        return int(bot_info().get("owner_id") or OWNER_ID)
    except:
        return OWNER_ID

def bot_username():
    global ME
    if BOT_USERNAME and BOT_USERNAME != "__BOT_USERNAME__":
...
BOT_COMMANDS = {
    "ا", "ايدي", "الاوامر", "الأوامر", "اوامر", "سورس", "المطور", "نقاطي",
    "لغز", "عربي", "اعلام", "سيارات", "مشاهير", "المشاهير", "كت", "كت تويت",
    "لو خيروك", "الاسرع", "غنيلي", "قران", "تراتيل", "افلام", "ترند", "ر",
    "فلوسي", "حسابي", "راتب", "بخشيش", "همسه", "همسة", "هم"
}

def needs_sub(text):
    return text in BOT_COMMANDS or text.startswith("يوت ") or text.startswith("تشغيل ") or text.startswith("طقس") or text.startswith("بيع نقاطي")
...
def rank_level(chat_id, user_id):
    return RANK_LEVEL.get(get_rank(chat_id, user_id), 0)

def is_group_admin(chat_id, user_id):
    if user_id in [OWNER_ID, owner_id()]:
        return True
    try:
...
def is_admin_msg(message):
    return bool(message.from_user and (is_group_admin(message.chat.id, message.from_user.id) or rank_level(message.chat.id, message.from_user.id) >= 1))

def is_creator_basic(message):
    return bool(message.from_user and (is_group_admin(message.chat.id, message.from_user.id) or rank_level(message.chat.id, message.from_user.id) >= 5))
...
def user_mention(user):
    ...
    return f"@{username}" if username else f"<a href='tg://user?id={user.id}'>{user.first_name}</a>"

def bank(uid):
    uid = sid(uid)
    data["money"].setdefault(uid, 1000)
    data["bank_accounts"].setdefault(uid, random.randint(100000, 999999))
    return data["money"][uid]

def save_bank(uid, amount):
    data["money"][sid(uid)] = amount
    save_json(DATA_FILE, data)
...
def check_sub(message):
    if message.from_user and not is_subscribed(message.from_user.id):
        ch = force_channel()
        raw_send(message.chat.id, "⚠️ لازم تشترك بالقناة أولاً", {
            "inline_keyboard": [[blue("اشترك بالقناة", url="https://t.me/" + ch.replace("@", ""))]]
        }, message.message_id)
        return False
    return True

def save_media(message):
    if message.chat.type not in ["group", "supergroup"] or message.content_type not in ["photo", "video", "sticker", "animation"]:
        return
    gid = sid(message.chat.id)
    data["media_messages"].setdefault(gid, [])
    data["media_messages"][gid].append({"id": message.message_id, "type": message.content_type})
    data["media_messages"][gid] = data["media_messages"][gid][-1000:]
    save_json(DATA_FILE, data)
...
def punish(message, lock_key):
    ...
    if action == "mute":
        ...
        try:
            ...
            data["kicked"].setdefault(gid, [])
            data["kicked"][gid].append(sid(message.from_user.id))
            save_json(DATA_FILE, data)
        except:
            pass
...
def is_lock(chat_id, key):
    return data["locks"].get(sid(chat_id), {}).get(key, False)

def detect_violation(message, text):
    if message.chat.type not in ["group", "supergroup"] or not message.from_user or is_admin_msg(message):
        return None
    if is_lock(message.chat.id, "chat"):
        return "chat"
    cmap = {
        "photo": "photo", "video": "video", "sticker": "sticker", "animation": "animation",
        "document": "document", "voice": "voice", "audio": "audio", "contact": "contact", "video_note": "video_note"
    }
    key = cmap.get(message.content_type)
    if key and is_lock(message.chat.id, key):
        return key
    if is_lock(message.chat.id, "links") and URL_RE.search(text):
        return "links"
    if is_lock(message.chat.id, "channels") and ("t.me/" in text or "@" in text):
        return "channels"
    if is_lock(message.chat.id, "tag") and "@" in text:
        return "tag"
    if is_lock(message.chat.id, "username") and "@" in text:
        return "username"
    if is_lock(message.chat.id, "english") and EN_RE.search(text):
        return "english"
    if is_lock(message.chat.id, "persian") and PERSIAN_RE.search(text):
        return "persian"
    if is_lock(message.chat.id, "bad_words") and any(w in text.lower() for w in BAD_WORDS):
        return "bad_words"
    if is_lock(message.chat.id, "long") and len(text) > 350:
        return "long"
    if is_lock(message.chat.id, "markdown") and any(x in text for x in ["[", "](", "`", "*", "__"]):
        return "markdown"
    if is_lock(message.chat.id, "forward") and getattr(message, "forward_date", None):
        return "forward"
    return None
...
def show_id(message):
    ...
    title = data["titles"].get(gid, {}).get(uid, "لا يوجد")
...
def list_top(message):
    gid = sid(message.chat.id)
    items = sorted(data["messages"].get(gid, {}).items(), key=lambda x: x[1], reverse=True)[:10]
    if not items:
        return bot.reply_to(message, "ماكو تفاعل بعد")
    out = "- ترند المجموعة\n\n"
    for i, (uid, count) in enumerate(items, 1):
        out += f"{i}- {display_user(uid)} ↢ {count} رسالة\n"
    return bot.reply_to(message, out)

def tag_users(message, only_admins=False):
    gid = sid(message.chat.id)
    ids = list(data["messages"].get(gid, {}).keys())
    if only_admins:
        ids = [uid for uid in ids if rank_level(message.chat.id, int(uid)) >= 1 or is_group_admin(message.chat.id, int(uid))]
    if not ids:
        return bot.reply_to(message, "ماكو أعضاء مخزنين")
    chunk = ""
    for uid in ids[:80]:
...
def clean_media(message, types):
    if not is_creator_basic(message):
        return
    gid = sid(message.chat.id)
    saved = data["media_messages"].get(gid, [])
    deleted = 0
    keep = []
    for item in saved:
        if item["type"] in types:
            try:
                bot.delete_message(message.chat.id, item["id"])
                deleted += 1
            except:
                pass
        else:
            keep.append(item)
    data["media_messages"][gid] = keep
    save_json(DATA_FILE, data)
    bot.reply_to(message, f"تم تنظيف {deleted} رسالة")
...
def handle_waiting(message, text):
    ...
    if not w:
        return False
    step = w.get("step")
    gid = w.get("chat", sid(message.chat.id))
    if step == "set_id_template":
        data["id_template"][gid] = text
        data["waiting"].pop(uid, None)
        save_json(DATA_FILE, data)
        bot.reply_to(message, "تم تعيين كليشة الايدي")
        return True
    if step == "add_reply_word":
        data["waiting"][uid] = {"step": "add_reply_answer", "chat": gid, "word": text}
        save_json(DATA_FILE, data)
        bot.reply_to(message, "ارسل الرد")
        return True
    if step == "add_reply_answer":
        data["replies"].setdefault(gid, {})
        data["replies"][gid][w["word"]] = text
        data["waiting"].pop(uid, None)
        save_json(DATA_FILE, data)
    ...
    if step == "del_reply":
        data["replies"].setdefault(gid, {})
        data["replies"][gid].pop(text, None)
        data["waiting"].pop(uid, None)
        save_json(DATA_FILE, data)
        bot.reply_to(message, "تم حذف الرد")
        return True
    if step == "add_tag_name":
        data["waiting"][uid] = {"step": "add_tag_user", "chat": gid, "name": text}
        save_json(DATA_FILE, data)
        bot.reply_to(message, "ارسل اليوزر")
        return True
    if step == "add_tag_user":
        data["tags"].setdefault(gid, {})
        data["tags"][gid][w["name"]] = text
        data["waiting"].pop(uid, None)
        save_json(DATA_FILE, data)
        bot.reply_to(message, "تم إضافة التاك")
        return True
    if step == "whisper_text":
        whisper_id = str(int(time.time())) + str(random.randint(100, 999))
        data["waiting_games"][whisper_id] = {
            "type": "whisper",
            "from": message.from_user.id,
            "from_name": message.from_user.first_name,
            "to": w["to"],
            "to_name": w.get("to_name", str(w["to"])),
            "text": text,
            "chat": w["chat"]
        }
        data["waiting"].pop(uid, None)
        save_json(DATA_FILE, data)
        kb = {"inline_keyboard": [[blue("رؤية الهمسة", f"whisper_{whisper_id}")]]}
        raw_send(w["chat"], f"• همسة من {message.from_user.first_name} إلى {w.get('to_name', display_user(w['to']))}", kb)
...
def start(message):
    ...
    if uid in data["waiting"] and data["waiting"][uid].get("step") == "whisper_text":
        return bot.send_message(message.chat.id, "• أرسل رسالة الهمسة الآن وستصل فقط للشخص المحدد.")
...
def callbacks(call):
    ...
    if call.data == "commands":
        return raw_edit(call.message.chat.id, call.message.message_id, commands_text(), commands_kb())
    ...
    if call.data.startswith("quiz_"):
        ...
        if not game:
            msg = bot.send_message(call.message.chat.id, "انتهت اللعبة")
            delete_after(call.message.chat.id, msg.message_id, 2)
            return
        ...
        msg = bot.send_message(call.message.chat.id, "❌ إجابة خاطئة")
        delete_after(call.message.chat.id, msg.message_id, 2)
        return
    if call.data.startswith("whisper_"):
        wid = call.data.replace("whisper_", "")
        w = data.get("waiting_games", {}).get(wid)
        if not w or w.get("type") != "whisper":
            return bot.answer_callback_query(call.id, "• الهمسة غير موجودة", show_alert=True)
        if call.from_user.id != w["to"]:
            try:
                warn = "• شخص حاول فتح همستك\n\n"
                warn += f"- الاسم: {call.from_user.first_name}\n"
                warn += f"- الايدي: {call.from_user.id}\n"
                warn += f"- اليوزر: @{call.from_user.username or 'ماكو'}"
                raw_send(w["from"], warn)
            except:
                pass
            return bot.answer_callback_query(call.id, "• الهمسة لا تخصك", show_alert=True)
...
def handler(message):
    ...
    if message.chat.type == "private" and handle_waiting(message, text):
        return
    if message.chat.type != "private" and needs_sub(text) and not check_sub(message):
        return
    if message.chat.type != "private" and handle_waiting(message, text):
        return
    ...
    if violation:
        punish(message, violation)
        return

    gid = sid(message.chat.id)

    if text in data["replies"].get(gid, {}):
        return bot.reply_to(message, data["replies"][gid][text])
    if text in data["tags"].get(gid, {}):
        return bot.reply_to(message, data["tags"][gid][text])

    if text in ["الاوامر", "الأوامر", "اوامر"]:
        return raw_send(message.chat.id, commands_text(), commands_kb())
    if text == "سورس":
        return raw_send(message.chat.id, "• سورس فادي", {"inline_keyboard": [[red(SOURCE_NAME, url=f"https://t.me/{DEV_USERNAME}")]]})
    if text == "المطور":
        return raw_send(message.chat.id, "• حساب المطور", {"inline_keyboard": [[blue("المطور", url=f"https://t.me/{DEV_USERNAME}")]]})
    ...
    if text.startswith("قفل ") or text.startswith("فتح "):
        ...
        if "بالكتم" in rest:
            lock_action = "mute"
            rest = rest.replace("بالكتم", "").strip()
        elif "بالطرد" in rest:
            lock_action = "kick"
            rest = rest.replace("بالطرد", "").strip()
        elif "بالتقييد" in rest:
            lock_action = "restrict"
            rest = rest.replace("بالتقييد", "").strip()
        key = LOCK_TYPES.get(rest)
        if not key:
            return bot.reply_to(message, "هذا القفل غير موجود")
        data["locks"].setdefault(gid, {})
        data["lock_actions"].setdefault(gid, {})
        data["locks"][gid][key] = action == "قفل"
        data["lock_actions"][gid][key] = lock_action
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم " + ("قفل " if action == "قفل" else "فتح ") + rest)

    if text == "تثبيت" or text == "ت":
        if not is_admin_msg(message) or not message.reply_to_message:
            return
        try:
            bot.pin_chat_message(message.chat.id, message.reply_to_message.message_id)
            return bot.reply_to(message, "تم تثبيت الرسالة")
        except:
            return bot.reply_to(message, "ما اكدر اثبت")
    if text == "الغاء تثبيت":
        if not is_admin_msg(message):
            return
        try:
            bot.unpin_chat_message(message.chat.id)
            return bot.reply_to(message, "تم الغاء التثبيت")
        except:
            return

    if text == "م" and message.reply_to_message:
        text = "رفع مميز"
    if text == "اد" and message.reply_to_message:
        text = "رفع ادمن"
    if text.startswith("رفع ") or text.startswith("تنزيل "):
        ...
        act, rank = text.split(maxsplit=1)
        rank = rank.strip()
        if rank not in RANK_NAMES:
            return
        data["ranks"].setdefault(gid, {})
        if act == "رفع":
            data["ranks"][gid][sid(u.id)] = rank
            save_json(DATA_FILE, data)
            return bot.reply_to(message, f"تم رفعه {rank}")
        data["ranks"][gid].pop(sid(u.id), None)
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم تنزيله")

    if text == "نزلني":
        data["ranks"].setdefault(gid, {})
        data["ranks"][gid].pop(sid(message.from_user.id), None)
        save_json(DATA_FILE, data)
    ...
    if text == "صلاحياتي":
        try:
            ...
            attrs = [("حذف الرسائل", "can_delete_messages"), ("حظر المستخدمين", "can_restrict_members"), ("تقييد المستخدمين", "can_restrict_members"), ("تثبيت الرسائل", "can_pin_messages"), ("دعوة المستخدمين", "can_invite_users"), ("تغيير معلومات المجموعة", "can_change_info"), ("إضافة مشرفين", "can_promote_members")]
            lines = [f"- {n}: {'✓' if getattr(m, a, False) else '✗'}" for n, a in attrs]
            uid = sid(message.from_user.id)
            ...
            return bot.reply_to(message, "ما اكدر اجيب صلاحياتك")

    if text == "اضف رد":
        if not is_admin_msg(message):
            return
        data["waiting"][sid(message.from_user.id)] = {"step": "add_reply_word", "chat": gid}
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "ارسل الكلمة")
    if text in ["حذف رد", "مسح رد"]:
        if not is_admin_msg(message):
            return
        data["waiting"][sid(message.from_user.id)] = {"step": "del_reply", "chat": gid}
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "ارسل الكلمة")
    if text == "الردود":
        arr = list(data["replies"].get(gid, {}).keys())
        return bot.reply_to(message, "• الردود المضافة:\n\n" + ("\n".join("- " + x for x in arr) if arr else "ماكو ردود"))

    if text == "اضف تاك":
        if not is_admin_msg(message):
            return
        data["waiting"][sid(message.from_user.id)] = {"step": "add_tag_name", "chat": gid}
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "ارسل الاسم")
    if text.startswith("مسح تاك "):
        if not is_admin_msg(message):
            return
        name = text.replace("مسح تاك ", "", 1).strip()
        data["tags"].setdefault(gid, {})
        data["tags"][gid].pop(name, None)
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم حذف التاك")
    ...
    if text.startswith("اضف نقاط ") and message.reply_to_message:
        ...
        uid = sid(message.reply_to_message.from_user.id)
        data["points"][uid] = data["points"].get(uid, 0) + amount
        save_json(DATA_FILE, data)
        return bot.reply_to(message, f"تم إضافة {amount} نقطة")
    if text == "نقاطي":
    ...
    if text.startswith("بيع نقاطي"):
        ...
        data["messages"].setdefault(gid, {})
        data["messages"][gid][uid] = data["messages"][gid].get(uid, 0) + amount * 3
        save_json(DATA_FILE, data)
        return bot.reply_to(message, f"تم بيع {amount} نقطة\nتمت إضافة {amount * 3} رسالة")

    if text in ["تعيين الايدي", "تغيير الايدي"]:
        if not is_admin_msg(message):
            return
        data["waiting"][sid(message.from_user.id)] = {"step": "set_id_template", "chat": gid}
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "ارسل كليشة الايدي\nالمتغيرات:\n{name}\n{id}\n{rank}\n{title}\n{messages}\n{points}")
    if text in ["ا", "ايدي"]:
        return show_id(message)
    if text == "تفع":
        data["settings"]["id_photo"] = True
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم تفعيل ايدي بالصورة")
    if text == "تعط":
        data["settings"]["id_photo"] = False
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم تعطيل ايدي بالصورة")
    ...
    if text == "ضع رابط":
        ...
        try:
            ...
            return bot.reply_to(message, "ما اكدر اسوي رابط")
    if text == "ترند":
        return list_top(message)
    if text == "تصفير الترند":
        if not is_creator_basic(message):
            return
        data["messages"][gid] = {}
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم تصفير الترند")
    if text == "تاك للكل":
        if not is_admin_msg(message):
            return
    ...
    if text == "تنظيف ميديا":
        return clean_media(message, ["photo", "video", "sticker", "animation"])
    if text == "تنظيف الصور":
        return clean_media(message, ["photo"])
    if text == "تنظيف الفيديوهات":
        return clean_media(message, ["video"])
    if text == "تنظيف الملصقات":
        return clean_media(message, ["sticker"])
    if text == "تنظيف المتحركات":
        return clean_media(message, ["animation"])
    ...
    if text == "قائمه المنع":
        arr = data["blocked_words"].get(gid, [])
        return bot.reply_to(message, "\n".join(arr) if arr else "ماكو كلمات ممنوعة")
    if text.startswith("منع "):
        if not is_admin_msg(message):
            return
        word = text.replace("منع ", "", 1).strip()
        data["blocked_words"].setdefault(gid, [])
        if word not in data["blocked_words"][gid]:
            data["blocked_words"][gid].append(word)
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم منع الكلمة")
    if text.startswith("الغاء منع "):
        if not is_admin_msg(message):
            return
        word = text.replace("الغاء منع ", "", 1).strip()
        try:
            data["blocked_words"][gid].remove(word)
        except:
            pass
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم الغاء المنع")
    ...
    if text in ["انشاء حساب بنكي", "انشاء"]:
        bank(message.from_user.id)
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم إنشاء حسابك البنكي")
    if text == "مسح حساب بنكي":
        data["money"].pop(sid(message.from_user.id), None)
        data["bank_accounts"].pop(sid(message.from_user.id), None)
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم مسح حسابك البنكي")
    if text == "حسابي":
        bank(message.from_user.id)
        return bot.reply_to(message, f"رقم حسابك: <code>{data['bank_accounts'][sid(message.from_user.id)]}</code>")
    if text == "فلوسي":
        return bot.reply_to(message, f"فلوسك: {bank(message.from_user.id)}$")
    if text == "راتب":
        ok, left = cooldown(message.from_user.id, "salary", 1200)
        if not ok:
            return bot.reply_to(message, f"انتظر {left} ثانية")
        money = bank(message.from_user.id) + random.randint(500, 1500)
        save_bank(message.from_user.id, money)
        return bot.reply_to(message, f"تم استلام راتبك\nفلوسك: {money}$")
    if text == "بخشيش":
        ok, left = cooldown(message.from_user.id, "tip", 600)
        if not ok:
            return bot.reply_to(message, f"انتظر {left} ثانية")
        money = bank(message.from_user.id) + random.randint(100, 500)
        save_bank(message.from_user.id, money)
        return bot.reply_to(message, f"بخشيش وصل\nفلوسك: {money}$")
    if text in ["زرف", "سرقه", "سرقة"] and message.reply_to_message:
        ok, left = cooldown(message.from_user.id, "rob", 600)
        if not ok:
            return bot.reply_to(message, f"انتظر {left} ثانية")
        victim = message.reply_to_message.from_user
        amount = min(bank(victim.id), random.randint(50, 500))
        save_bank(victim.id, bank(victim.id) - amount)
        save_bank(message.from_user.id, bank(message.from_user.id) + amount)
        data["robbers"][sid(message.from_user.id)] = data["robbers"].get(sid(message.from_user.id), 0) + 1
        save_json(DATA_FILE, data)
        return bot.reply_to(message, f"زرفته {amount}$")
    if text.startswith("استثمار ") or text.startswith("حظ ") or text.startswith("مضاربه "):
        nums = re.findall(r"\d+", text)
        if not nums:
            return
        amount = int(nums[0])
        bal = bank(message.from_user.id)
        if bal < amount:
            return bot.reply_to(message, "فلوسك ما تكفي")
        if text.startswith("استثمار "):
            profit = int(amount * random.randint(1, 15) / 100)
            save_bank(message.from_user.id, bal + profit)
            return bot.reply_to(message, f"ربحت {profit}$")
        if text.startswith("حظ "):
            if random.choice([True, False]):
                save_bank(message.from_user.id, bal + amount)
                return bot.reply_to(message, f"فزت {amount}$")
            save_bank(message.from_user.id, bal - amount)
            return bot.reply_to(message, f"خسرت {amount}$")
        percent = random.randint(-90, 90)
        change = int(amount * percent / 100)
        save_bank(message.from_user.id, bal + change)
        return bot.reply_to(message, f"النسبة: {percent}%\nالنتيجة: {change}$")
    if text == "توب الفلوس":
        items = sorted(data["money"].items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    if text == "المالك":
        try:
            ...
            if owner:
                return bot.reply_to(message, f"• مالك المجموعة:\n- الاسم: {owner.first_name}\n- اليوزر: @{owner.username or 'ماكو'}\n- الايدي: <code>{owner.id}</code>")
        except:
            pass
        return bot.reply_to(message, "ما قدرت اجيب المالك")
    if text == "لقبي":
        return bot.reply_to(message, f"• لقبك: {data['titles'].get(gid, {}).get(sid(message.from_user.id), 'لا يوجد')}")
    if text.startswith("اضف لقب ") and message.reply_to_message:
        if not is_admin_msg(message):
            return
        title = text.replace("اضف لقب ", "", 1).strip()
        data["titles"].setdefault(gid, {})
        data["titles"][gid][sid(message.reply_to_message.from_user.id)] = title
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم إضافة اللقب")
    if text in ["نسبة الحب", "نسبة الكره", "نسبة الرجوله", "نسبة الانوثه", "نسبة الجمال"]:
        return bot.reply_to(message, f"{text}: {random.randint(0,100)}%")
    if text in ["بوسه", "بوسها"] and message.reply_to_message:
        return bot.reply_to(message, f"💋 {message.from_user.first_name} باس {message.reply_to_message.from_user.first_name}")
    if text == "ز":
        ids = [x for x in list(data["messages"].get(gid, {}).keys()) if x != sid(message.from_user.id)]
        if not ids:
            return bot.reply_to(message, "ماكو أعضاء")
        partner = random.choice(ids)
        data["marriages"][sid(message.from_user.id)] = partner
        save_json(DATA_FILE, data)
        return bot.reply_to(message, f"مبروك زوجتك {display_user(partner)}")
    if text in ["زوجتي", "زوجي"]:
        p = data["marriages"].get(sid(message.from_user.id))
        return bot.reply_to(message, f"زوجتك/زوجك: {display_user(p)}" if p else "انت أعزب")
    if text == "ط":
        data["marriages"].pop(sid(message.from_user.id), None)
        save_json(DATA_FILE, data)
        return bot.reply_to(message, "تم الطلاق")
    if text == "زواج" and message.reply_to_message:
        price = min(bank(message.from_user.id), 500)
        data["marriages"][sid(message.from_user.id)] = sid(message.reply_to_message.from_user.id)
        save_bank(message.from_user.id, bank(message.from_user.id) - price)
        return bot.reply_to(message, f"تم الزواج\nالمهر: {price}$")

    if text in ["همسه", "همسة", "هم"] and message.reply_to_message:
        if message.chat.type == "private":
            return
        if message.reply_to_message.from_user.id == message.from_user.id:
            return bot.reply_to(message, "لا تقدر تهمس لنفسك.")
        data["waiting"][sid(message.from_user.id)] = {
            "step": "whisper_text",
        ...
        save_json(DATA_FILE, data)
        kb = {"inline_keyboard": [[blue("اضغط هنا لاكمال الهمسه", url=f"https://t.me/{bot_username()}?start=whisper")]]}
        return bot.reply_to(message, f"• تم تحديد الهمسة لـ {message.reply_to_message.from_user.first_name}\n• اضغط اسفل لاكمال الهمسة", reply_markup=kb)
    ...
    if text == "قران":
        s, rec, url = random.choice(QURAN)
        return bot.send_audio(message.chat.id, url, title=s, performer=rec, caption=f"📖 {s}\nالقارئ: {rec}")
    if text == "تراتيل":
        name, url = random.choice(CHANTS)
        return bot.reply_to(message, f"🎼 {name}\nارفع روابط التراتيل الصحيحة داخل القائمة حتى يرسلها صوت.")
    if text == "غنيلي":
        return bot.reply_to(message, random.choice(["🎵 يا ليل يا عين", "🎶 غنيتلك من گلبي", "🎧 اكتب: يوت اسم الأغنية"]))
